 ...
 */
const updateHaliSahaStats = async (haliSahaId) => {
    try {
...
const createReview = async (req, res) => {
    logger_1.default.info("createReview çağrıldı by user %s", req.user?.id);
...
const updateReview = async (req, res) => {
    const { id } = req.params;
    logger_1.default.info("updateReview çağrıldı – id: %s by user %s", id, req.user?.id);
    ...
    try {
        const existingReview = await prisma_1.prisma.review.findUnique({ where: { id } });
        if (!existingReview) {
        ...
        if (existingReview.userId !== req.user?.id && req.user?.role !== "ADMIN") {
            logger_1.default.warn("updateReview: yetkisiz erişim by %s on %s", req.user?.id, id);
...
const deleteReview = async (req, res) => {
    const { id } = req.params;
    logger_1.default.info("deleteReview çağrıldı – id: %s by user %s", id, req.user?.id);
    try {
        const review = await prisma_1.prisma.review.findUnique({ where: { id } });
        if (!review) {
        ...
        if (review.userId !== req.user?.id && req.user?.role !== "ADMIN") {
            logger_1.default.warn("deleteReview: yetkisiz erişim by %s on %s", req.user?.id, id);
            (0, apiResponse_1.sendResponse)(res, httpStatusCode_1.HttpStatusCode.FORBIDDEN, {
                success: false,
